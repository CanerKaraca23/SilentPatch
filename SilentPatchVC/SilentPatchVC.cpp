...
void InjectDelayedPatches_VC_Common( bool bHasDebugMenu, const wchar_t* wcModulePath )
{
	...
	try
	{
		auto pat1 = pattern("68 A0 F9 03 00 6A ? 68 ? ? ? ?");
		if (pat1.size() > 0) {
			ScriptSpace = *pat1.get(0).get<unsigned char*>(8);
		} else {
			ScriptSpace = AddressByVersion<unsigned char*>(0x821280, 0x821288, 0x820288);
		}
	}
	TXN_CATCH();
	...
	if (ScriptSpace)
	{
		try
		{
			...
			ReadCall(the_scripts_load, orgTheScriptsLoad);
			InjectHook(the_scripts_load, TheScriptsLoad_HardwareShopsCleanupFix);

			auto fread_scripts = get_pattern("68 A0 F9 03 00 6A 01 68 ? ? ? ? E8", 12);
			ReadCall(fread_scripts, org_fread_scripts);
			InjectHook(fread_scripts, fread_scripts_Hook);
		}
		TXN_CATCH();
	}
